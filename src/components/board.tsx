  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(
        "http://localhost:3001/api/board/1/cards"
      );
      setCardData(response.data);
      setBacklogColumnData(
        response.data.filter(
          (card: ColumnCard) => card.column.toUpperCase() === "BACKLOG"
        )
      );
      setToDoColumnData(
        response.data.filter(
          (card: ColumnCard) => card.column.toUpperCase() === "TODO"
        )
      );
      setDoingColumnData(
        response.data.filter(
          (card: ColumnCard) => card.column.toUpperCase() === "DOING"
        )
      );
      setDoneColumnData(
        response.data.filter(
          (card: ColumnCard) => card.column.toUpperCase() === "DONE"
        )
      );
    };
    fetchData();
